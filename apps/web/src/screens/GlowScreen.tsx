...
const SUB_TABS: { id: GlowSubTab; label: string; color: string }[] = [
  { id: 'ascend',  label: 'ASCEND',  color: '#F472B6' },
  { id: 'routine', label: 'ROUTINE', color: '#4ADE80' },
  { id: 'metrics', label: 'METRICS', color: '#38BDF8' },
];
...
const ALL_TIPS: Tip[] = [
  { icon: '💧', tag: 'SKIN',  color: '#38BDF8', title: 'Hydration = glass skin',           body: 'Drink 35ml per kg bodyweight. Dehydration shows in your face within hours — dull skin, puffy eyes, visible pores.' },
  { icon: '🥩', tag: 'SKIN',  color: '#38BDF8', title: 'Collagen starts with protein',     body: 'Your body can\'t make collagen without glycine and proline. Lean beef, chicken skin and bone broth are the top sources.' },
  { icon: '🧴', tag: 'SKIN',  color: '#38BDF8', title: 'SPF every single morning',         body: 'UV is the #1 cause of premature aging. SPF 30+ daily is more effective than any serum or supplement.' },
  { icon: '🫐', tag: 'SKIN',  color: '#38BDF8', title: 'Antioxidants block UV damage',     body: 'Vitamin C and E neutralize free radicals. Blueberries, bell peppers and almonds keep your skin barrier strong.' },
  { icon: '🌙', tag: 'SKIN',  color: '#38BDF8', title: 'Night is when skin repairs',       body: 'Growth hormone peaks at 11pm–1am and drives collagen rebuild. Sleep deprivation visibly ages skin in 48 hours.' },
  { icon: '🥚', tag: 'HAIR',  color: '#FBBF24', title: 'Eggs are the best hair food',      body: 'Whole eggs give biotin, zinc, selenium and protein — every micronutrient hair follicles need. 2 eggs a day is enough.' },
  { icon: '🐟', tag: 'HAIR',  color: '#FBBF24', title: 'Omega-3 thickens hair',            body: 'DHT (the hair-loss hormone) is suppressed by omega-3. Salmon, sardines and walnuts are the highest sources.' },
  { icon: '🌿', tag: 'HAIR',  color: '#FBBF24', title: 'Iron deficiency causes shedding',  body: 'Low ferritin is the most overlooked cause of hair loss. Red meat, spinach, lentils. Get bloodwork if shedding.' },
  { icon: '💊', tag: 'HAIR',  color: '#FBBF24', title: 'Scalp massage 4 min/day',          body: 'Daily scalp massage increases hair growth by 68% in studies. Do it in the shower — no cost, measurable result.' },
  { icon: '😴', tag: 'SLEEP', color: '#A78BFA', title: '8h sleep = free HGH injection',    body: 'HGH peaks in the first 90 min of deep sleep. It builds muscle, burns fat and repairs collagen simultaneously.' },
  { icon: '🌡️', tag: 'SLEEP', color: '#A78BFA', title: 'Cold room = deep sleep',           body: 'Core temp must drop 1–2°C to reach deep sleep. Keep your room at 18–19°C. Warm shower 1h before bed triggers the drop.' },
  { icon: '📵', tag: 'SLEEP', color: '#A78BFA', title: 'Screen light delays sleep 90min',  body: 'Blue light suppresses melatonin for 90 minutes. Switch to warm light or blue-light glasses after 9pm.' },
  { icon: '💪', tag: 'BODY',  color: '#4ADE80', title: 'Posture is the free jawline hack', body: 'Forward head posture collapses your jawline. 2 min of dead hangs + chin tucks daily resets your anterior chain.' },
  { icon: '🧊', tag: 'BODY',  color: '#4ADE80', title: 'Cold exposure boosts testosterone', body: 'Cold showers 3–5 min daily increase norepinephrine by 300% and support testosterone levels.' },
  { icon: '🪒', tag: 'GROOM', color: '#F97316', title: 'Skin prep > razor quality',         body: 'Soften stubble 90 sec under hot water before shaving. A cheap razor with good prep beats an expensive one used dry.' },
  { icon: '🧴', tag: 'GROOM', color: '#F97316', title: 'Moisturizer within 60 seconds',    body: 'Apply within 60 seconds of washing your face — while skin is still damp. Locks in hydration 3x more effectively.' },
];
...
function buildNudges(
  ...
  if (strengthSessions >= 4) {
    n.push({
      icon: '🏋️', color: '#9D7EFF', priority: 3,
  ...
  if (protLeft > 25 && cal > 0) {
    ...
    n.push({
      icon: '🥩', color: '#38BDF8', priority: isWorkout ? 1 : 2,
  ...
  if (calDiff > 300) {
    ...
    n.push({
      icon: '🍽️', color: '#FBBF24', priority: 3,
  ...
  if (isRest) {
    ...
    n.push({ icon: '🛌', color: '#A78BFA', priority: 4, title: 'Rest day protocol', body: restBody });
  }

  // ── Late carbs ───────────────────────────────────────────────────────────────
  if (isEvening && targets && carb > targets.carbsG * 0.45 && cal > 0) {
    n.push({
      icon: '🌙', color: '#A78BFA', priority: 3,
  ...
  if (age && age >= 28 && weeklyRunKm === 0 && strengthSessions === 0) {
    n.push({
      icon: '⏱️', color: '#F97316', priority: 5,
  ...
  if (isMorning) {
    n.push({
      icon: '☀️', color: '#FBBF24', priority: 6,
...
export default function GlowScreen() {
  ...
  const recColor = wl.recoveryScore >= 70 ? '#4ADE80' : wl.recoveryScore >= 40 ? '#FBBF24' : '#F87171';
  const fatColor = wl.legFatigueScore >= 70 ? '#F87171' : wl.legFatigueScore >= 40 ? '#FBBF24' : '#4ADE80';
      ...
      <div className="card a a2" style={{ padding: '16px 18px', marginBottom: 20 }}>
        ...
        {targets && cal > 0 && (
          <div style={{ display: 'flex', gap: 14 }}>
            <ScoreBar label="Calories" value={calPct}  color={calPct > 105 ? '#F87171' : '#4ADE80'} />
            <ScoreBar label="Protein"  value={protPct} color={protPct >= 90 ? '#4ADE80' : '#38BDF8'} />
          </div>
        )}
        <div style={{ display: 'flex', gap: 16, marginTop: 14, flexWrap: 'wrap' }}>
          {[
            { label: 'Run km', val: `${wl.totalRunKm.toFixed(0)}km`, color: '#38BDF8' },
            { label: 'Strength', val: `${wl.strengthSessions.length}x`, color: '#9D7EFF' },
            tl?.trainingType ? { label: 'Today', val: tl.trainingType, color: 'var(--accent)' } : null,
            tl?.dailyActivityModifier ? { label: 'Steps', val: tl.dailyActivityModifier, color: '#FBBF24' } : null,
