...
const inp: React.CSSProperties = {
  ...
  fontSize: 15, padding: '14px 15px', outline: 'none', marginBottom: 10,
...
export default function ProfileSetupScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await updateProfile({ weightKg: w, heightCm: h, age: a, gender, activityLevel });
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: '#0C0C0C' }}>
      ...
      <div className="nrc-a nrc-a1" style={{ padding: '28px 20px 0', marginBottom: 24 }}>
        ...
        {!profileComplete && (
          <div style={{ color: '#333', fontSize: 13, marginTop: 10, fontWeight: 500, lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            {[
              ...
              <div key={label} className="nrc-press" style={{
                ...
                <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2, color: '#333', textTransform: 'uppercase', marginBottom: 2 }}>{label}</div>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#2A2A2A', letterSpacing: 1 }}>{unit}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding: '0 20px 40px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
          {(['male', 'female'] as const).map((g) => (
            <button key={g} onClick={() => setGender(g)} className="nrc-press" style={{
              ...
              borderColor: gender === g ? RED : '#222',
              background: gender === g ? 'rgba(255,28,28,0.08)' : '#141414',
              color: gender === g ? RED : '#444',
          ...
          {ACTIVITY_LEVELS.map(({ value, label, desc }) => {
            ...
            return (
              <button key={value} onClick={() => setActivityLevel(value)} className="nrc-press" style={{
                ...
                background: active ? '#141414' : 'rgba(255,255,255,0.02)',
                border: '1px solid', borderColor: active ? RED : '#1A1A1A',
                borderLeft: active ? `3px solid ${RED}` : '1px solid #1A1A1A',
                transition: 'all 0.15s',
              }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: active ? RED : '#333', marginBottom: 3, letterSpacing: -0.3 }}>{label}</div>
                <div style={{ fontSize: 12, color: active ? '#444' : '#2A2A2A', fontWeight: 500 }}>{desc}</div>
              </button>
            );
          })}
        </div>
        ...
        <button onClick={handleSave} disabled={saving} className="nrc-press" style={{
          ...
          background: saved ? 'rgba(34,197,94,0.1)' : saving ? '#141414' : RED,
          color: saved ? '#22c55e' : saving ? '#333' : '#fff',
        ...
        {/* Account */}
        <div style={{ background: '#141414', borderRadius: 16, padding: '16px 18px', border: '1px solid #1E1E1E' }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 3, color: '#2A2A2A', textTransform: 'uppercase', marginBottom: 10 }}>Account</div>
          <div style={{ color: '#333', fontSize: 13, marginBottom: 16 }}>{user?.email}</div>
          <button onClick={() => { if (window.confirm('Sign out?')) logout(); }} className="nrc-press" style={{
